...
$(document).ready( function () {
	...
	$(".button-edit-location").click(function () {
		$button_edit=$(this);
		...
		if(!$("#tr-edit-"+id).length){
			// select delete label
			var $delete_label = $("#delete-location-"+id);
	...
	$("#button-new-location").click(function () {
		...
		$(this).parent().parent().before("<tr class='tr-new'>"
			// removal button
			+ "<td><button id='button-remove-location-"+new_locations+"' type='button'>x</button></td>"
			+ "<td></td><td></td><td><input type='text' name='description[]' ></input></td>"
			+" </tr>");
		
		// add function to remove button
		$("#button-remove-location-"+new_locations).click(function () {
			$(this).parent().parent().remove();
		});		
