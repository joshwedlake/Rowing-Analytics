...
$(document).ready( function () {
	...
	$(".button-edit-sport").click(function () {
		$button_edit=$(this);
		...
		if(!$("#tr-edit-"+id).length){
			// select delete label
			var $delete_label = $("#delete-sport-"+id);
	...
	$("#button-new-sport").click(function () {
		// get parent
		$(this).parent().parent().before("<tr class='tr-new'>"
			// removal button
			+ "<td><button id='button-remove-sport-"+new_sports+"' type='button'>x</button></td>"
			+ "<td></td><td></td><td><input type='text' name='description[]' ></input></td>"
			+" </tr>");
		
		// add function to remove button
		$("#button-remove-sport-"+new_sports).click(function () {
			$(this).parent().parent().remove();
		});		
